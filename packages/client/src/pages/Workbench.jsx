...
function Workbench() {
  ...
  // Calculate current level index based on URL param or default to 0
  ...
  const handleLevelChange = (e) => {
    ...
    const newLevelId = levels[newIndex].id;
    navigate(`/level/${newLevelId}`);
  };

  const handleNextLevel = () => {
    if (currentLevelIdx < levels.length - 1) {
      navigate(`/level/${levels[currentLevelIdx + 1].id}`);
    }
  };
  ...
  const handleCompile = async () => {
    ...
    try {
      const jobName = `level_${currentLevel.id}_${sessionId}`;
      ...
      if (success) {
        ...
        if (currentLevel.check(code)) {
          ...
          const currentMax = parseInt(localStorage.getItem('latex_max_level') || '1');
          
          if (nextLevelId > currentMax && nextLevelId <= levels.length) {
            localStorage.setItem('latex_max_level', nextLevelId.toString());
          }
        } else {
          ...
          >
            {levels.map((lvl, index) => (
              <option key={lvl.id} value={index}>
                Level {index + 1}: {lvl.title.replace(/^\d+\.\s*/, '')}
              </option>
            ))}
          </select>
        </div>
